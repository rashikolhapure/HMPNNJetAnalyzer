    ...
    **kwargs
    ...
    if "color" in kwargs:
        ...
        for item in np.unique(colors):
            print(
                item, np.unique(colors)
            )
            if (
                item
                not in color_to_label
            ):
                continue
            extra = Rectangle(
                (0, 0),
                ...
                edgecolor="none",
                linewidth=0,
                label=color_to_label[
                    item
                ],
            )
            p.axes.add_patch(extra)
    p.axes.set_xlim(
        kwargs.pop("xlim", (0.5, 1.0))
    )
    p.axes.barh(
        index, data, 1.0, **kwargs
    )
