...
def get_from_indices(
    ...
    """
    Extract selected keys and corresponding indices from a dictionary of events.

    Args:
        events (dict): A dictionary containing event data.
        indices (array-like): Indices to select from the event data.
        keys (list of str, optional): A list of keys to extract from the events. If None, all keys are extracted.

    Returns:
        dict: A new dictionary containing the selected keys and their corresponding values.
    """
    if keys is None:
...
def get_delphes(run_names, **kwargs):
    ...
    Args:
        run_names (str or list of str): The name(s) of the Delphes run(s) to load.
        **kwargs: Additional keyword arguments for DelphesNumpy initialization.
...
def get_numpy_events(run_name, runs="first", **kwargs):
    ...
    Args:
        ...
        runs (str, optional): Specify how to handle multiple runs if present ('first' to return the first run,
            'merge' to merge all runs). Default is 'first'.
