    ...
    def __init__(self, run_name, *args, **kwargs):
        ...
        if kwargs.get("root_file_path") is None:
            self.in_data = RootEvents(
                run_name,
                return_vals=("Delphes", "path", "index"),
                select_runs=kwargs.get("select_runs", []),
                tag=kwargs.get("root_file_tag", ""),
                ignore_runs=kwargs.get("ignore_runs", []),
            )
        else:
            self.in_data = RootEvents(
                run_name,
        ...
        self.out_data = NumpyEvents(
            ...
            prefix=kwargs.get("root_file_tag", ""),
        )
        ...
        for key, val in self.final_state_attributes.items():
            print(f"{key:15}  {val}\n")
            ...
            try:
                ...
                for final_state in self.final_state_attributes:
                    ...
                    current[final_state] = self.get(
                        final_state, self.final_state_attributes[final_state]
                    )
                ...
                if self.extract_gen_particles:
                    ...
                    current["Particle"] = self.get(
                        "Particle", self.final_state_attributes["Particle"]
                    )
                return current
        else:
            raise StopIteration
...
class BaselineCuts(PhysicsMethod):
    def __init__(self, run_name, *args, **kwargs):
        ...
        self.in_data = NumpyEvents(
            ...
            prefix=kwargs.get("delphes_prefix", ""),
            both_dirs=kwargs.get("both_dirs", False),
            select_runs=kwargs.get("select_runs", []),
            ignore_runs=kwargs.get("ignore_runs", []),
        )
...
if __name__ == "__main__":
    now = DelphesNumpy(
        "wwx",
        tag="try",
        reconstructed_objects={"FatJet": 2, "Tower": None},
        image_shapes={"FatJet": (32, 32), "Tower": (100, 100)},
    )
