...
def read_lhe(path='.',filename="unweighted_events.lhe",final_state_only=True,
    ...
    if add_attribute:
        ...
        if return_structured: return np.array(events),np.array(event_attributes)
        else: return events,event_attributes
    if return_structured: return np.array(events)
    else: return events
...
def convert_to_dict(events,final_states=None,return_vector=True,name=True,sort=True):
    ...
    for event in events:
        ...
        for key,val in current.items():
            if sort: val=ru.Sort(np.array(val))
