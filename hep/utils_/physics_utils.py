...
def ConvertToLhc(Array):
    ...
    for i in range(len(Array)):
        ReturnArray[i]=np.array([-np.log(np.tan(np.arccos(Array[i,3]/np.sqrt(Array[i,1]**2+Array[i,2]**2+Array[i,3]**2))/2.)),np.arctan(Array[i,2]/Array[i,1]),np.sqrt(Array[i,1]**2+Array[i,2]**2),Array[i,0]])
