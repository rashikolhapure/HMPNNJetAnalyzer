...
def GetTLorentzVector(
    ...
    Convert a numpy.ndarray of elements to TLorentzVector(s) with
    the specified format.

    Parameters:
    ----------
    array : numpy.ndarray
        The input array of elements to be converted to
        TLorentzVector(s).
    format : str, optional
        The desired output format. Choose from
        {"fatjet", "lhc", "lorentz", "image"}.
    particle : str, optional
        The type of particle for TLorentzVector construction
        (default is "visible").
    ...
    >>> elements = [np.array([30.0, 1.2, 0.5, 0.0]),
    ...             np.array([40.0, -0.8, 2.1, 0.0])]
    ...
    if format == "fatjet":
        ...
        for item in array:
            ...
            count += 1
        return vec
    # assert format in ("lhc","lorentz")
    # and (array.shape[-2]==3 or array.shape[-1]==4)
...
def GetNumpy(
    ...
    observable_first : bool, optional
        Whether the resulting array should have the observable
        as the first axis (default is True).
    ...
    >>> vectors = [TLorentzVector(1.0, 2.0, 3.0, 4.0),
    ...            TLorentzVector(5.0, 6.0, 7.0, 8.0)]
...
def Sort(
    ...
    attribute : str, optional
        The attribute by which to sort the TLorentzVectors. Options are
        ("p", "pt", "px", "py", "pz", "eta", "phi", "mass", "E").
    order : str, optional
        The sorting order, "desc" for descending (default) or "asc" for
        ascending.
    ...
    Example:
    --------
    To sort an array of TLorentzVectors by transverse momentum (pt)
    in descending order:

    >>> vectors = np.array([TLorentzVector(1.0, 2.0, 3.0, 4.0),
    ...                     TLorentzVector(5.0, 6.0, 7.0, 8.0)])
...
def Print(
    ...
    format : str, optional
        ...
        "lhc" (default) or "other."
    name : str, optional
        A custom name for the printed content.

    Notes:
    -----
    This function can print information about a single TLorentzVector
    or a list of TLorentzVectors.
    If 'format' is set to "lhc," it prints Eta, Phi, Pt, Mass,
    and Momentum (P).
    If 'format' is set to "other," it prints Px, Py, Pz, Energy (E),
    and Momentum (P).
    ...
    if isinstance(vector, TLorentzVector):
        if format == "lhc":
            print(
                f"    Eta: {vector.Eta():20.16f}   Phi: {vector.Phi():20.16f}\
                      Pt : {vector.Pt():20.16f}    Mass: {vector.M():20.16f}\
                      P= {vector.P():20.16f}"
            )
        else:
            print(
                f"    Px: {vector.Px():20.16f}   Py: {vector.Py():20.16f}\
                      Pz : {vector.Pz():20.16f}    E: {vector.E():20.16f}\
                      P={vector.P():20.16f}"
            )
    else:
        ...
        for item in vector:
            if format == "lhc":
                print(
                    f"    Eta: {item.Eta():20.16f}   Phi: {item.Phi():20.16f}\
                          Pt : {item.Pt():20.16f}    Mass: {item.M():20.16f}\
                          P= {item.P():20.16f}"
                )
            else:
                print(
                    f"    Px : {item.Px():20.16f}   Py: {item .Py():20.16f}\
                          Pz: {item.Pz():20.16f}    E : {item.E():20.16f}\
                          P= {item.P():20.16f}"
                )
    return
...
def Broadcast(
    ...
    check : bool, optional
        Whether to check the broadcasting by comparing sums
        (default is False).
    ...
    Notes:
    -----
    This function takes a list of fatjets, and for each fatjet,
    it appends an empty TLorentzVector to the end of the fatjet.
    It returns the resulting array of fatjets.

    If 'check' is True, it also performs a check by comparing the
    sums of the original and broadcasted fatjets.
    """
    return_array = []
