...
def process_fatjets(fatjets, operation="all", subparts="subjets", **kwargs):
    ...
    for fatjet_index, fatjet in enumerate(fatjets):
        ...
        try:
            if subjets[1][2] < 0:
                fatjet, subjets = reflect(fatjet, subjets)
        except IndexError:
            pass
...
def regularize_fatjet(
    ...
    **kwargs
    ...
    if inclusive:
        ...
        subjets = np.array(
            [
                TLorentzVector(item.px, item.py, item.pz, item.e)
                for item in FatJet().Recluster(
                    fatjet, r=recl_r, algorithm=recl_algo, subjets=n_subjets, pt_min=5
                )
            ]
        )
    else:
        recl_r = kwargs.get("recluster_r", r)
        sequence, _ = FatJet().Recluster(
            fatjet,
            r=recl_r,
            algorithm=recl_algo,
            subjets=n_subjets,
        ...
        subjets = np.swapaxes(
            np.array(
                [
                    [item.eta, item.phi, item.pt]
                    for item in FatJet().Recluster(
                        fatjet, r=recl_r, algorithm=recl_algo, subjets=n_subjets
                    )
                ]
            ),
            0,
...
def _remove_jets(
    ...
    if not return_jets:
        ...
        return lorentz_tower, shifted_jets
...
def tower_bin(tower, format="tower", **kwargs):
    ...
    if kwargs.get("return_seperate", False):
        ...
        left_bin = binner(
            np.array([tower_left[:, 1], tower_left[:, 2], tower_left[:, 0]]),
            x_interval=(-5, center_range[0]),
        ...
        return binner(
            np.array([tower[:, 1], tower[:, 2], tower[:, 0]]),
            x_interval=(-5, 5),
            y_interval=(-np.pi, np.pi),
            bin_size=bin_size,
            swap=True,
...
def binner(
    ...
    **kwargs
    ...
    if "shape" in kwargs:
        ...
        x_bin_size, y_bin_size = (x_interval[1] - x_interval[0]) / shape[0], (
