...
def remove_jets(lorentz_tower,lorentz_jets,r=0.4,return_jets=False,shift_jets=True,**kwargs):
...
def _remove_jets(lorentz_tower,lorentz_jets,r=0.4,**kwargs):
    if kwargs.get("verbose",False):
        print ("Removing jet constituents...")
        print (lorentz_tower.shape)
    return_array=[]
...
def tower_padding_(tower,pad_axis=0,pad_size=4):
    """"wrong one for cylinder, valid for mobius strip"""
    if pad_axis==0: 
        new_shape=(tower.shape[0]+2*pad_size,tower.shape[1])
        return_array=np.zeros(new_shape)
        return_array[pad_size:-pad_size,:]=tower
        ...
        raise ValueError("Not edited for axis=1")
        new_shape=(tower.shape[0],tower.shape[1]+2*pad_size)
        return_array=np.zeros(new_shape)
        return_array[:,pad_size:-pad_size]=tower
        for i in range(pad_size):
...
def tower_bin(tower,format="tower",**kwargs):
    ...
    if kwargs.get("return_seperate",False):
        ...
        left_bin=binner(np.array([tower_left[:,1],tower_left[:,2],tower_left[:,0]]),x_interval=(-5,-1.6),y_interval=(-np.pi,np.pi),bin_size=bin_size,swap=True)
        center_bin=binner(np.array([tower_center[:,1],tower_center[:,2],tower_center[:,0]]),x_interval=(-1.6,1.6),y_interval=(-np.pi,np.pi),bin_size=bin_size,swap=True)
        right_bin=binner(np.array([tower_right[:,1],tower_right[:,2],tower_right[:,0]]),x_interval=(1.6,5),y_interval=(-np.pi,np.pi),bin_size=bin_size,swap=True)
        if "plot" in sys.argv:
