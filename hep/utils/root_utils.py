    ...
    if len(array.shape) == 1:
        ...
        if format == "lhc":
            if len(array) == 4:
                ...
                mass = 0.0
            if particle == "MET":
                vec.SetPtEtaPhiM(array[0], 0.0, array[2], 0.0)
            else:
                vec.SetPtEtaPhiM(array[0], array[1], array[2], mass)
        else:
...
def GetNumpy(
        ...
        if type(vectors) == TLorentzVector:
            ...
            for i in range(len(vectors)):
                return_array[i] = np.array(
                    [vectors[i].Eta(), vectors[i].Phi(), vectors[i].Pt()],
                    dtype="float64",
                )
    elif format == "lhc":
        if type(vectors) == TLorentzVector:
            if exclude_mass:
                return np.array(
                    [vectors.Pt(), vectors.Eta(), vectors.Phi()], dtype="float64"
                )
            else:
                return np.array(
                    [vectors.Pt(), vectors.Eta(), vectors.Phi(), vectors.M()],
                    dtype="float64",
                )
        else:
            if exclude_mass:
                ...
                for i in range(len(vectors)):
                    ...
                    if diff is None:
                        return_array[i] = np.array(
                            [vectors[i].Pt(), vectors[i].Eta(), vectors[i].Phi()],
                            dtype="float64",
                        )
                    else:
                        if include_del_r:
    ...
    if observable_first:
        return np.swapaxes(return_array, 0, 1)
    else:
        return return_array
