...
def Boost(particle, direction, eta):
    ...
    E, p = particle[0], Euclid3Norm(
        particle[1:] * direction
    )
    # print (particle,E,p)
    return np.array(
        [
...
def SumCombinations(
    FourVectors, Map=None, comb=2
):
    assert (
        len(FourVectors.shape) == 2
        and FourVectors.shape[1] == 4
...
def Get3Direction(FourVector):
    ...
    Dir = FourVector[1:] / Euclid3Norm(
        FourVector
    )
    assert (
        abs(Euclid3Norm(Dir) - 1)
        < 1e-12
    )
    return Dir
...
def Euclid3Norm(FourVector):
    if (
        len(FourVector.shape) == 1
        and len(FourVector) == 3
    ):
        FourVector = np.concatenate(
            ([0.0], FourVector), axis=0
        )
        return np.sqrt(
            np.sum(
                FourVector[:3] ** 2,
