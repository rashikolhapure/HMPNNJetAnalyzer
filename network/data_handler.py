...
def pad_values(events,target_shapes,value=0):
    for key,new_shape in target_shapes.items():
        ...
        if "plot" in sys.argv:
            ...
            p.fig,p.axes=fig,axes[0]
            p.Image(old_val[10])
        if old_shape[1] < new_shape[0]:
            diff=new_shape[0]-old_shape[1]
            x_new=np.full((old_shape[0],diff,old_shape[2]),value)
            old_val=np.concatenate((x_new[:,:int(diff/2)],old_val,x_new[:,int(diff/2):]),axis=1)
        old_shape=old_val.shape
        if old_shape[2] < new_shape[1]:
            diff=new_shape[1]-old_shape[2]
            x_new=np.full((old_shape[0],old_shape[1],diff),value)
            #print (x_new.shape,old_val.shape)
            old_val=np.concatenate((x_new[:,:,:int(diff/2)],old_val,x_new[:,:,int(diff/2):]),axis=2)
        if "plot" in sys.argv:
            p.axes=axes[1]
            p.Image(old_val[10])
            p.save_fig("zero_pad",extension="eps")
        #print (old_val.shape)
        events[key]=old_val
    return events
...
def load_data(classes,length=30000,preprocess_tag=None,test_train_split=0.25,input_keys=["high_level"],return_array=False,function=None,run_io=False,**kwargs):
    ...
    target_shapes=kwargs.get("target_shapes",{})
    if "sim_in" in input_keys: 
        assert "sim_central_values" in kwargs
        random=kwargs.get("random",True)
        sim_central_values=kwargs.get("sim_central_values")
    ...
    for item in classes:
        if not run_io:
            if function is None:
                ...
                events=Unpickle(item+".h",load_path="./processed_events/"+preprocess_tag+folder)
        ...
        if "network_in" in input_keys:
            events["network_in"]=Unpickle(item+".h",load_path="./processed_events/network_out")["network_out"]
        if target_shapes: events=pad_values(events,target_shapes)
        for i,input_key in enumerate(input_keys):
            if input_key=="tower_image":
                ...
                central_values=kwargs.get("sim_central_values")
                if random:
                    X[i]=np.random.normal(loc=sim_central_values[item]["mean"],scale=sim_central_values[item]["scale"],size=tuple([length]+list(sim_central_values["shape"])))
                else: 
                    X[i]=np.full(tuple([length]+list(sim_central_values["shape"])),sim_central_values[item]["mean"])
                print ("sim_in shape: ",X[i].shape)         
            elif input_key=="num_bins":
                X[i]=np.zeros((events["tower_image"][:length].shape[0],1))
                for ind,item in enumerate(events["tower_image"][:length]):
                    X[i][ind,0]=len(np.where(events["tower_image"][ind])[0])
            ...
            if normalize:
                ...
                for ind in range(len(X[i])): X[i][ind]=X[i][ind]/np.sqrt(np.sum(X[i][ind]))
                print ("\nNormalized:\n min:",np.min(X[i])," max: ",np.max(X[i])) 
    ...
    if len(input_keys)==1: 
        ...
        X_train,X_val,Y_train,Y_val,train_indices,test_indices=train_test_split(X_all,Y_all,indices,shuffle=True,random_state=12,test_size=0.25)
    else:
        x_length=len(X_all)
    ...
    train_dict={"X":X_train,"Y":Y_train,"indices":train_indices}
    test_dict={"X":X_val,"Y":Y_val,"indices":test_indices}
...
if __name__=="__main__":
    ...
    X_train,Y_train,X_test,Y_test=load_data(classes,input_keys=["tower_image"],suffix="low_res_tower_jet_phi",return_array=True,length=30000,target_shapes={"tower_image":(48,64)})
