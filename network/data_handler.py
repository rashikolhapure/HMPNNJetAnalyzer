    ...
    **kwargs
    ...
    for item in classes:
        ...
        if "network_in" in input_keys:
            events["network_in"] = Unpickle(
                item + ".h", load_path="./processed_events/network_out"
            )["network_out"]
        if target_shapes:
            events = pad_values(events, target_shapes)
        for i, input_key in enumerate(input_keys):
            if input_key == "tower_image":
                ...
                if random:
                    ...
                    X[i] = np.full(
                        tuple([length] + list(sim_central_values["shape"])),
                        sim_central_values[item]["mean"],
                    )
...
if __name__ == "__main__":
    ...
    X_train, Y_train, X_test, Y_test = load_data(
        ...
        return_array=True,
        length=30000,
        target_shapes={"tower_image": (48, 64)},
    )
