...
class ModelData(object):
    ...
    def network_input(
        ...
        if len(self.input_states) == 1:
            ...
            for (
                f_state
            ) in self.input_states:
                if (
                    type(f_state.index)
                    == int
                ):
                    listed[
                        f_state.network_input_index
                    ] = final_state_dict[
                        f_state
                    ][
                        :,
    ...
    def write_to_disk(self):
        assert (
            self.mode == "w"
        ), "Trying to write in read instance of class"
        print(
            "Saving to: ",
            self.data_save_path,
        )
    ...
    def check_consistency(
        self, final_states_dict
    ):
        print(
            "First load! Checking consistency..."
        )
        for (
            input_state
        ) in self.input_states:
    ...
    def multiple_input_check(self):
        print(
            self.train_data.keys()
        )  # ,self.train_data["Y"][-10:])
        if (
            type(self.train_data["X"])
            != list
        ):
            self.train_data = (
                nu_shuffle(
    ...
    def load_from_index_dict(
        ...
        all_count = 0
        if (
            "train" not in index_dict
            and "val" not in index_dict
        ):
            for (
                key,
                value,
    ...
    def get_data(self):
        if {
            ...
            if type(train_X) != list:
                ...
                container_train = array_shuffle(
                    all=container_train
                )
                container_val = array_shuffle(
                    all=container_val
                )
                train_data = {
                    "Y": container_train[
...
class DataHandler(object):
    ...
    def get_data(self):
        if "dictionary" in os.listdir(
            ...
            save = True
        self.load_path = data.pop(
            "path"
        )
        train, val = data.pop(
            "train"
        ), data.pop("val")
        self.train_indices = train.pop(
            "indices"
        )
        self.val_indices = val.pop(
            "indices"
        )
        if save:
            self.write_to_disk()
