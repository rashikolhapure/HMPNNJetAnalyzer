...
from ..genutils import print_events,dir_ext_count
...
class Deform(Operator):
    def __init__(self,*args,**kwargs):
        ...
        self.operation=None
        self.relevance={"hard":("hard_scale"),"soft_deform":("num_pixels","soft_scale","deform_scale"),
                        "local_soft":("num_pixels","soft_scale","deform_scale"),"hard_deform":("hard_scale","num_pixels","deform_scale"),
                        "local":("num_pixels","deform_radius","deform_scale")}
        self.ht_local=False
    def unconstrained_deform(self,array,indices):
        deformed=np.zeros(array.shape,dtype="float64")
        x,y,_=np.where(array)
        try: def_inds=np.random.choice(indices,self.num_pixels,replace=False)
        except ValueError: def_inds=indices
        undef_inds=np.array([int(i) for i in range(len(x)) if i not in def_inds])
        x_def,y_def=x[def_inds],y[def_inds]
        try:x_undef,y_undef=x[undef_inds],y[undef_inds]
        except IndexError: pass
        else: deformed[x_undef,y_undef]=array[x_undef,y_undef]
        x_deformed=[np.random.randint(i-self.deform_scale,i+self.deform_scale)%array.shape[0] for i in x_def ]
        y_deformed=[np.random.randint(i-self.deform_scale,i+self.deform_scale)%array.shape[1] for i in y_def ]
        deformed[x_deformed,y_deformed]=deformed[x_deformed,y_deformed]+array[x_def,y_def]
        if "debug" in sys.argv:debug_logger(array,deformed,x_def,y_def) 
        return deformed
    def constrained_deform(self,def_inds,allowed_indices):
        pass
    def local(self,array):
        ...
        for i in range(len(x)):
            if np.sqrt((x[i]-center_ind[0])**2+(y[i]-center_ind[1])**2)<=self.deform_radius+0.2:
                inds.append(i)
    ...
    def local_soft(self,array):
        ...
        count=0
        for i in range(len(x)):
            if np.sqrt((x[i]-x[center_ind])**2+(y[i]-y[center_ind])**2)<=self.deform_radius+0.2:
                inds.append(i)
        print (inds)
        ht_sum=np.sum(array[x[inds],y[inds]])
        ...
        sys.exit()
        try: def_inds=np.random.choice(soft_inds,self.num_pixels,replace=False)
        except ValueError: def_inds=soft_inds
        deformed=self.unconstrained_deform(array,inds)
    def soft_deform(self,array):
        if self.num_pixels==0: return array
        ht_sum=np.sum(array)
        if "debug" in sys.argv: 
            print ("ht_sum: ",ht_sum,"cut_off :",self.soft_scale*ht_sum,array[np.where(array)])
        x,y,_=np.where(array)
        deformed=np.zeros(array.shape,dtype="float64")
    ...
    def hard_deform(self,array):
        ...
        if "debug" in sys.argv: print (self.hard_scale*ht_sum)
    ...
    def hard(self,array):
        ...
        return hard_array
def debug_logger(array,deformed,x_def,y_def): 
    print ("deform values: \n",array[x_def,y_def,0],"\nvalues in same position of deformed array: \n",deformed[x_def,y_def,0])
    print ("values adjacent to deform values\n")
    print ("west: \n    original \n    ",array[x_def-1,y_def,0],x_def-1,y_def,"\n    deformed :\n    ",deformed[x_def-1,y_def,0],x_def-1,y_def)
    print ("east: \n    original \n    ",array[x_def+1,y_def,0],x_def+1,y_def,"\n    deformed :\n    ",deformed[x_def+1,y_def,0],x_def+1,y_def)
    print ("south: \n    original \n    ",array[x_def,y_def-1,0],x_def,y_def-1,"\n    deformed :\n    ",deformed[x_def,y_def-1,0],x_def,y_def-1)
    print ("north: \n    original \n    ",array[x_def,y_def+1,0],x_def,y_def+1,"\n    deformed :\n",deformed[x_def,y_def+1,0],x_def,y_def+1)
    print ("north-east: \n    original \n    ",array[x_def+1,y_def+1,0],x_def,y_def+1,"\n    deformed :\n    ",deformed[x_def+1,y_def+1,0])
    print ("north-west: \n    original \n    ",array[x_def-1,y_def+1,0],x_def-1,y_def+1,"\n    deformed :\n    ",deformed[x_def-1,y_def+1,0],x_def-1,y_def+1) 
    print ("south-east: \n    original \n    ",array[x_def+1,y_def-1,0],x_def+1,y_def-1,"\n    deformed :\n    ",deformed[x_def+1,y_def-1,0],x_def+1,y_def-1)
    print ("south-west: \n    original \n    ",array[x_def-1,y_def-1,0],x_def-1,y_def-1,"\n    deformed :\n    ",deformed[x_def-1,y_def-1,0],x_def-1,y_def-1)
...
def operator(run_name,operation_name=None,operation_class=None,parameter_name=None,parameter_values=None,roc_plot=False,**kwargs):
    ...
    for item in kwargs: 
        ...
        operation_class.__setattr__(item,kwargs.get(item))
        print (item,operation_class.__getattribute__(item))
    ...
    for i,parameter in enumerate(parameter_values):
        ...
        print (operation_name,parameter_name,operation_class.__getattribute__(parameter_name))
        if "debug" in sys.argv: operation_class.iterator(I.unoperated_data)
        if parameter in roc_plot_values: roc_plot=True
        else: roc_plot=False
        temp_dict=I.predict(operation=operation_class.iterator,roc_plot=roc_plot)
        if roc_plot: 
            roc_data=temp_dict.pop("roc")
            roc_data["specs"]={"parameter_name":parameter_name,"parameter_value":parameter,"operation_name":type(operation_class).__name__}
            for item in kwargs: roc_data["specs"][item]=kwargs.get(item)
            Pickle(roc_data,"roc_"+tag+"_"+str(parameter)+".h",save_path=I.inference_data)
        if i==0: 
            #break
            ...
            for _ in temp_dict: acc_dict[_].append(temp_dict[_][-1])
    ...
    count=len([item for item in os.listdir(I.inference_data) if item.startswith(filename) and item.endswith(".h")])
    Pickle(acc_dict,filename+str(count)+".h",save_path=I.inference_data)
    for item in acc_dict: print (item,acc_dict[item])
    return os.path.join(I.inference_data,filename+str(count)+".h")
