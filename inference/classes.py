...
class Inference(NetworkMethod):
    ...
    def model_data_load(self):
        try:
            data = Unpickle(self.tag + ".pickle", load_path=self.data_path)
            ...
            try:
                index = Unpickle("index_dict.pickle", load_path=self.data_path)
    ...
    def seperate_classes(self, data):
        ...
        return {
            self.class_names[0]: {"X": X0, "Y": Y0},
            self.class_names[1]: {"X": X1, "Y": Y1},
        }
    ...
    def predict(
        ...
        for i, item in enumerate(class_names):
            ...
            if i == 0:
                ...
                y_true, y_pred = np.concatenate(
                    (y_true, seperated[item]["Y"][:, 0]), axis=0
        ...
        if save:
            Pickle(saved_dict, "prediction", save_path=self.data_save_path)
        if roc_plot:
