...
from ..genutils import check_file,check_dir,pool_splitter,print_events
...
class Inference(NetworkMethod):
    def __init__(self,model_name,tag="val",**kwargs):
        self.tag=tag
        self.extra_handler_kwargs=kwargs.get('extra_handler_kwargs',{})
        ...
        self.run_path=os.path.join(self._prefix_path,self.run_tag,self.preprocess_tag,self.model_name)
        ...
        assert os.path.exists(self.run_path) and os.path.exists(self.data_path) and os.path.exists(self.model_checkpoints)
        ...
        self.get_model=kwargs.get("get_model",False)
        self.all_data=None
        ...
        if not self.get_model:
            ...
            else: self.unoperated_data=self.model_data_load()
    ...
    def replace_data(self,data,**kwargs):
        ...
        out_key=kwargs.get("out_key",self.class_names[-1])
        preprocess_tag=kwargs.get("data_tag",self.preprocess_tag)
        input_keys=kwargs.get("input_keys",self.input_keys)
        new_data=Unpickle(new_class+".h",load_path="./processed_events/"+preprocess_tag+"/all/") 
        ...
        for i,key in enumerate(self.input_keys):
            if key=="tower_image":
                X_new[i]=np.expand_dims(new_data["tower_image"],-1)
            else: X_new[i]=new_data[key]
            length=len(X_new[i])
        if len(input_keys)==1: X_new=X_new[0]
    ...
    def model_data_load(self):
        try: data=Unpickle(self.tag+".pickle",load_path=self.data_path)
        except Exception as e:
            print (e,"\nCould not load "+self.tag+".pickle from ",self.data_path,"\ntrying to load through index dicts...")
            data=self.load_from_index_dict(self.data_path)
        else:
            try:index=Unpickle("index_dict.pickle",load_path=self.data_path)
            except FileNotFoundError: self.class_names=("class 0","class 1")
            else:
                class_names=[]
        ...
        finally: print (self.class_names)
        return data
    def seperate_classes(self,data):
        ...
        if not self.class_names: self.class_names=("class_0","class_1")
        return {self.class_names[0]:{"X":X0,"Y":Y0},self.class_names[1]:{"X":X1,"Y":Y1}} 
    def choose_model(self,model_files):
        val_acc=[]
        remove_inds=[]
        for i,item in enumerate(model_files):
            try: val=eval(item[:-len(".hdf5")].split("_")[-1])
            except NameError: 
                print (item," no val_acc in filename.")
                remove_inds.append(i)
                continue
            else: val_acc.append(val)
        model_files=[item for i,item in enumerate(model_files) if i not in remove_inds]
            #print (item,eval(item[:-len(".hdf5")].split("_")[-1]))
        val_acc=np.array(val_acc)
        if self.best=="high": ind=-1
        else: ind=0
        model_path=model_files[val_acc.argsort()[ind]]
        return model_path
    def get_best_model(self):
        model_files=check_file(".hdf5",self.model_checkpoints)
        model_path=self.choose_model(model_files)
        print ("Loading best model from path : ",model_path)
        model=keras.models.load_model(model_path)
        self.best_model_path=model_path
        self.model=model
        return model
    def get_best_from_seperate_runs(self):
        model_files=check_file(".hdf5",self.model_checkpoints)
        ...
        for item in per_run_files:
            ...
            print ("Loading model from path: ",best_current_path)
    ...
    def per_run_predict(self,**kwargs):
        if self.per_run_models is None: self.get_best_from_seperate_runs()
        global_model=self.model
        return_list=[]
        for item,path in zip(self.per_run_models,self.per_run_model_paths):
            self.model=item
            print ("\nPredicting with model loaded from: ",path)
            print ("Layer config of the model: ")
            for layer in item.layers: print (layer.get_config())
            item.summary()
        ...
        return return_list
    def predict(self,operation="None",split=False,roc_plot=False,replace=None,save=False,batch_size=300):
        assert self.unoperated_data is not None,"Initialized for extracting model, set get_model to False during\
        ...
        if operation=="None": 
            ...
            if "debug" in sys.argv or "operation_check" in sys.argv or not split: combined_data=operation(self.unoperated_data)
            combined_data=pool_splitter(operation,self.unoperated_data)
        seperated=self.seperate_classes(combined_data)
        class_names=self.class_names
        if replace:
            print ("replacing ",class_names[-1]," with :",replace)
            seperated=self.replace_data(seperated,new_class=replace)
            class_names=(self.class_names[0],replace)
            
        if roc_plot:roc_dict={"combined":self.model.predict(combined_data["X"],verbose=2)}
        ...
        if class_names[-1]==self.class_names[-1]:
            ...
            return_dict["combined"]=self.model.evaluate(x=combined_data["X"],y=combined_data["Y"],batch_size=batch_size,verbose=1) 
        ...
        for i,item in enumerate(class_names):
            ...
            print ("Evaluating class: ",item)
            self.model.evaluate(x=seperated[item]["X"],y=seperated[item]["Y"],batch_size=batch_size,verbose=1)
            return_dict[item]=self.model.predict(seperated[item]["X"],verbose=1)[:,0]
            return_dict[item+"_true"]=seperated[item]["Y"][:,0]
            if i==0: y_true,y_pred=seperated[item]["Y"][:,0],return_dict[item]
            else: y_true,y_pred=np.concatenate((y_true,seperated[item]["Y"][:,0]),axis=0),np.concatenate((y_pred,return_dict[item]),axis=0)
            saved_dict[item]=return_dict[item]
            if roc_plot:
                roc_dict[item]=self.model.predict(seperated[item]["X"],verbose=2)
                if save: Pickle(roc_dict[item],"roc_"+item,save_path=self.data_save_path)
        ...
        if "channel" in combined_data: return_dict["channel"]=combined_data["channel"]
        if save:Pickle(saved_dict,"prediction",save_path=self.data_save_path)
        if roc_plot:
