    ...
    for item in append:
        if item == "EventAttribute":
            ...
            append[item] = workaround_concatenate(
                append[item], temp[item][:append_length], return_type="list"
            )
            continue
        ...
        if type(append[item]) == np.ndarray and type(temp[item]) == np.ndarray:
            try:
                append[item] = np.concatenate((append[item], temp[item][:append_length]), axis=0)
            ...
            for i in range(len(append[item])):
                append[item][i] = np.concatenate(
                    (append[item][i], temp[item][i][:append_length]), axis=0
                )
            if "debug" in sys.argv:
                print(
                    "list",
    ...
    if verbose:
        for item in data:
            ...
            for particle in item:
                try:
                    print(particle, item[particle].shape)
                except AttributeError:
                    print(particle, len(item[particle]))
    return_dict = {}
    for item in data:
        for particle in item:
        ...
        for key in data[i]:
            ...
            if count == 0:
                ...
                if type(data[i][key]) == list:
                    ...
                    try:
                        return_dict[key] = np.concatenate((return_dict[key], data[i][key]), axis=0)
                    except ValueError as E:
                        print(
                            E,
