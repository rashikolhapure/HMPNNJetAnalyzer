        ...
        if type(append[item]) == np.ndarray and type(temp[item]) == np.ndarray:
            try:
                append[item] = np.concatenate(
                    (append[item], temp[item][:append_length]), axis=0
                )
            except ValueError as e:
                print(e, "trying workaround method")
                append[item] = workaround_concatenate(
                    append[item], temp[item][:append_length]
                )
...
def pool_splitter(
    ...
    if with_lock:
        ...
        p = multiprocessing.Pool(
            processes=num_cores, initializer=init_lock, initargs=(l,)
        )
    else:
                ...
                if type(data[i][key]) == list:
                    ...
                    try:
                        return_dict[key] = np.concatenate(
                            (return_dict[key], data[i][key]), axis=0
                        )
                    except ValueError as E:
                        print(
                            E,
