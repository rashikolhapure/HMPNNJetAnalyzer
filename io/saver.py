...
def Unpickle(
    ...
    **kwargs: additional keyword arguments to pass to the pickle.load function (e.g., encoding)
    ...
    if path is not None:
        load_path = path
    if verbose:
        print("Reading {} from {}".format(filename, load_path))
    if "." not in filename:
    ...
    if filename[-4:] == ".npy":
        ...
        if verbose:
            print(filename, " loaded from ", os.getcwd())
        os.chdir(pwd)
        return ret
    try:
...
def Pickle(
    ...
    if path is not None:
        save_path = path
    if verbose:
        print("Saving {} at {}".format(filename, save_path))
    pwd = os.getcwd()
    if save_path != ".":
        os.chdir(save_path)
    if not overwrite:
    ...
    if type(python_object) == np.ndarray:
        ...
        suffix = ""
    if verbose:
        print(filename + suffix, " saved at ", os.getcwd())
    os.chdir(pwd)
    return
...
if __name__ == "__main__":
    a = np.random.normal(loc=0.0, scale=5.0, size=(40, 3))
    print(a.shape)
